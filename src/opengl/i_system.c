...
//-----------------------------------------------------------------------------
/// \file
/// \brief SRB2 system stuff for dedicated servers (and the OpenGL backend)
...
static UINT8 keyState[GLFW_KEY_LAST + 1];
static double lastMouseX, lastMouseY;
static boolean mouseInit;
...
void I_GetEvent(void)
{
	...
	for (int k = 0; k <= GLFW_KEY_LAST; k++)
	{
		...
		else if (s == GLFW_RELEASE && keyState[k])
		{
			keyState[k] = 0;
	...
	ev.type = ev_mouse;
	ev.key = 0;
	ev.x = (INT32)(x - lastMouseX);
	ev.y = (INT32)(y - lastMouseY);
	ev.repeated = false;
...
void I_SetTextInputMode(boolean active)
{
	...
	if (window)
		glfwSetInputMode(window, GLFW_CURSOR,
			active ? GLFW_CURSOR_NORMAL : GLFW_CURSOR_DISABLED);
}
